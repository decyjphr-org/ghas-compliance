      ...
      return context.octokit.checks.update(
        context.repo({
          ...
          output: {
            ...
            summary: 'Skipped because runtime settings: skipCheckAlways=true',
      ...
      if (res.data.names.find(x => x === 'ncnia')) {
        return context.octokit.checks.update(
          context.repo({
            ...
            output: {
              title: 'GHAS Compliance Report',
              summary: 'Skipped because of \'ncnia\' app',
              text: 'GHAS Compliance check will be skipped'
            }
          })
        )
...
async function executeGHASComplianceCheck (policyManager, context, log) {
  ...
  // let policyPath = await context.config(PolicyManager.FILE_NAME)
  ...
  return secretScanningAlerts(policy, pullRequest, context, log).then((res) => {
...
async function codeScanningAlerts (policy, pullRequest, context, log) {
  ...
  return context.octokit.request('GET /repos/{owner}/{repo}/code-scanning/analyses',
    ...
    log.debug(`+++ ${filteredAlertsByLevel}`)
