...
[View Secret Scanning Alerts](${encodeURI(url)})`
    ...
    if (!res.analysisFound) {
      return completeCheckRun(context, 'completed', 'failure', {
        ...
        summary: `No Analysis found for tool CodeQL for the Branch ${ref}, and Commit ${sha}`,
        text: 'Please upload the codescanning results for the latest commit of the branch'
      })
    }
    ...
    const url = `https://github.com/${context.payload.repository.owner.login}/${context.payload.repository.name}/security/code-scanning?query=branch:${ref}+tool:CodeQL+is:open`
