...
const docsHost = 'https://docs.host.com'
...
async function executeGHASComplianceCheck (policyManager, context, log, ref, sha) {
  ...
  const docsurl = `${docsHost}/pages/`
  const ssdlcurl = `${docsHost}/Contacts`
...
## SUMMARY
### Policy
The following findings are considered Policy Failing and must be addressed to release to production, refer to [Docs](${docsurl}) for more details of Security Policy based on application type.
...
- Specific CWEs regardless of severity (refer to [Highway](${docsurl}) for details)
...
Reach out to [Security](${ssdlcurl}) team for guidance, as needed.
...
async function codeScanningAlerts (policyManager, ref, sha, context, log) {
  const policy = await policyManager.getPolicy()
  const pullRequest = context.payload.check_run.pull_requests[0]
  log.debug('Checking policy for code scanning alerts')
  // Check if there is an analysis for that sha
  let analysisFound = false
  const promises = []
  promises.push(
    context.octokit.request('GET /repos/{owner}/{repo}/code-scanning/analyses',
      context.repo({ ref: `refs/heads/${ref}` }))
  )
  if (pullRequest) {
    promises.push(
      context.octokit.request('GET /repos/{owner}/{repo}/code-scanning/analyses',
        context.repo({ ref: `refs/pull/${pullRequest.number}/head` }))
    )
  }
  ...
  return Promise.all(promises).then(res => {
    ...
    const analysis = res.length === 2 ? res[0].data.concat(res[1].data) : res[0].data
...
async function secretScanningAlerts (policyManager, context, log) {
  ...
  return context.octokit.paginate('GET /repos/{owner}/{repo}/secret-scanning/alerts',
    ...
    return { success: true, allSecrets: res, filteredSecrets }
  }).catch(e => {
    log.error(`Unknown error checking for secret scanning alerts ${e}`)
    return { success: false, reason: `${e}`, allSecrets: [], filteredSecrets: [] }
    /*
    if (e.status === 404) {
