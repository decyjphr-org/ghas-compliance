...
`
  ...
  async getPolicy () {
    try {
      ...
      const response = await this.context.octokit.repos.getContent(this.policyPath).catch(e => {
        this.log.warn(`Error getting policy from location ${JSON.stringify(this.policyPath)}`)
      })

      // Ignore in case path is a folder
      // - https://developer.github.com/v3/repos/contents/#response-if-content-is-a-directory
      if (!response || Array.isArray(response.data)) {
        this.log.warn(`Using default policy from the runtime config ${JSON.stringify(runtime.emptyPolicy)}`)
  ...
  filterRelevantAlerts (alertsList) {
    ...
    let alerts = alertsList.filter(alert => !ignores.includes(alert))
  ...
  filteredAlertsByConditions (alerts, ids, names, cwes) {
    let openAlerts = alerts.filter(alert => {
    ...
    openAlerts = openAlerts.filter(alert => !alertsByIds.includes(alert))
    ...
    openAlerts = openAlerts.filter(alert => !alertsByNames.includes(alert))
