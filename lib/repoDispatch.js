...
async function handleRepoDispatch (context, log) {
  ...
  if (clientPayload.createInitialCheck) {
    result = await createInitialCheck(context, log)
    ret.push(result)
  ...
  if (clientPayload.createBranchProtection) {
    result = await createBranchProtection(context, log)
    ret.push(result)
    if (result.status === 'error') {
      return ret
    }
  ...
  if (clientPayload.enableAdvSec) {
    result = await enableAdvSec(context, log)
    ret.push(result)
    return ret
  }
...
async function createInitialCheck (context, log) {
...
async function createBranchProtection (context, log) {
...
async function enableAdvSec (context, log) {
  const sec = {
