...
async function handleRepoDispatch (context, log) {
  ...
  log.debug(` ${JSON.stringify(clientPayload)}`)
...
async function complianceCheck (clientPayload, policyManager, context, log) {
  ...
  return context.octokit.git.getRef(context.repo({ ref: `refs/heads/${clientPayload.head_branch}` }))
    .then(commits => {
      return context.octokit.checks.create(
        context.repo({
          name: 'GHAS Compliance',
...
async function ghasEnable (clientPayload, policyManager, context, log) {
  const ret = []
  let result = null
  if (clientPayload.createInitialCheck) {
  ...
  if (clientPayload.createBranchProtection) {
...
async function createInitialCheck (policyManager, context, log) {
  if (!await policyManager.createInitialCheck()) {
    ...
    return { success: true, message: 'Skipping createInitialCheck runtime setting createInitialCheck=false' }
  }
...
async function createBranchProtection (policyManager, context, log) {
  if (!await policyManager.createBranchProtection()) {
    ...
    return { success: true, message: 'Skipping createBranchProtection runtime setting createBranchProtection=false' }
  }
...
async function enableAdvSec (policyManager, context, log) {
  if (!await policyManager.enableAdvSec()) {
    ...
    return { success: true, message: 'Skipping enableAdvSec runtime setting enableAdvSec=false' }
  }
  const sec = {
